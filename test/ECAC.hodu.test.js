...
beforeEach(async () => {
    ...
    // 주소 체크섬 오류 해결
    address = web3.utils.toChecksumAddress('0x3C0ea55D3A691B0C1cb12Cb8F23e5c7FdE798c9D')
    ...
    await box.methods.addUser().call();
...
describe('[Testcase 2 : check ECAC Smart Contract for addPoint_likes()]', () => {
    ...
    it('2.1. Is the function addPoint_likes() running correctly?', async () => {
        let result;
        ...
        try {
            ...
            // await box.methods.addPoint_likes(account, likes);
            // result = await box.methods.getCount(account).call();
            // console.log("The number of result: ", result);
            // 포인트가 제대로 지급되었는지 확인하기 위한 check_point += 100
            check_point += (Math.floor(likes/100)-reflectedLikes)*100;
        }
        catch (err) {
            console.log(err.message);
            console.log("==========에러 실행되고 있는가?==========");
        }
        // console.log("Here is the line 133", result);
    ...
    //     // 비교- 검증 부분 추가 (0 point -> 100 point 된 상황이어야함.)
